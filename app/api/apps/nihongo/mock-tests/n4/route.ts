...
import { calculateN4MockReadiness } from "@/lib/nihongo/mock-tests/readiness";
...
const sectionTargets = [
  { section: "Moji-Goi", take: 35 },
  { section: "Bunpo", take: 40 },
  { section: "Dokkai", take: 20 },
  { section: "Chokai", take: 5 },
];
...
export async function GET() {
  ...
  return NextResponse.json({
    ...
    totalAvailable: await prisma.nihongoMockTestQuestion.count({
      where: { level, testType: "mock_test", isActive: true },
    }),
  });
}
...
async function getAuthorizedUser() {
  ...
  const readiness = calculateN4MockReadiness({
    role: user.role,
  ...
  if (!readiness.isReady) {
    return {
      response: NextResponse.json(
        {
          error: "JLPT N4 Mock Test is locked.",
          readiness,
        },
        { status: 403 }
      ),
