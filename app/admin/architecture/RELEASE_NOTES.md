...
| --- | --- | --- | --- | --- |
| RN-2026.07.21-001 | 2026-07-21 00:00 +09:00 | Nexus Platform Team | UAT OK | Redesigned the Nexus AI Nihongo authenticated shell: removed the 224px sidebar/drawer for a fixed bottom nav (Home/Latihan/Progress/Lainnya sheet) on every breakpoint, slimmed the top header to one row, and rebuilt the dashboard as a single-column "story mode" home. Also fixed a rockstar-theme contrast bug where completed-lesson cards showed invisible white text on a light-mint background. `docs/DESIGN.md` bumped to version 2. Not yet pushed to production — see `docs/release-notes.md` 2026.07.21 entry. |
