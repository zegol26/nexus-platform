...
import {
  n4RehearsalSections,
  rehearsalCategories,
} from "@/app/apps/nihongo/full-rehearsal-n4/n4RehearsalData";
import type { RehearsalSection } from "@/app/apps/nihongo/full-rehearsal-n5/n5RehearsalData";

const storageKey = "nexus.nihongo.fullRehearsalN4.reviewed";

type CategoryFilter = RehearsalSection["category"] | "all";

export default function FullRehearsalN4Page() {
  ...
  const filteredSections = useMemo(() => {
    ...
    return n4RehearsalSections.filter((section) => {
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-8">
      ...
      <section className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm">
        <div className="bg-[linear-gradient(135deg,#0f172a,#155e75,#6d28d9)] p-6 text-white sm:p-8 lg:p-10">
          <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <h1 className="mt-3 text-4xl font-semibold leading-tight sm:text-5xl">
                Latihan Rehearsal Lengkap — JLPT N4
              </h1>
              <p className="mt-4 max-w-3xl text-sm leading-6 text-cyan-50">
                Review tata bahasa N4, kanji, reading strategy, komunikasi praktis, dan paket 100 soal dari modul.
              </p>
            </div>
            <Link
              href="/apps/nihongo/mock-test/n4"
        ...
        <div className="grid gap-4 p-6 sm:grid-cols-3 sm:p-8">
          <StatCard label="Total Section" value={`${n4RehearsalSections.length}`} />
          <StatCard label="Sudah Direview" value={`${reviewedIds.size}`} />
          <StatCard label="Level Fokus" value="JLPT N4" />
        </div>
      </section>
      ...
      <div className="grid gap-6 lg:grid-cols-[360px_1fr]">
        <aside className="space-y-6 lg:sticky lg:top-6 lg:h-fit">
          <RehearsalProgress
            ...
            totalCount={n4RehearsalSections.length}
          ...
          <section className="rounded-2xl border border-cyan-100 bg-cyan-50 p-5">
            ...
            <p className="mt-2 text-sm leading-6 text-slate-700">
              Review section inti dulu, lalu buka paket 100 soal di bagian akhir. Setelah stamina latihan terasa stabil,
              lanjutkan ke mock test N4.
            </p>
          </section>
        </aside>
