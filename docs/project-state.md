...
- Nexus AI Nihongo's authenticated app shell has been redesigned for a simpler, "story mode" feel: the 224px sidebar and its mobile drawer are removed from `app/apps/nihongo/layout.tsx` in favor of a single fixed bottom navigation (`NihongoBottomNav.tsx`) present at every breakpoint — Home, Latihan (curriculum), Progress (badges/mock readiness), and a "Lainnya" bottom sheet grouping the remaining secondary surfaces (flashcards, quiz, tutor, reading, listening, game, rehearsal, mock tests). The top header shrank to a single slim row. The dashboard (`NihongoDashboardClient.tsx`) was rebuilt as a single-column home: one hero continue-lesson CTA, a horizontal "Perjalanan belajar" path preview, and two compact status chips, replacing the prior dense multi-card/chart layout. `docs/DESIGN.md` is bumped to `version: 2` documenting the new Bottom Navigation component and updated Responsive Behavior table. A pre-existing `rockstar` theme bug was also fixed: the theme had no `emerald-*` color translation, so "completed" cards rendered near-invisible white text on a light-mint background; `app/globals.css` now has a rockstar emerald mapping mirroring the existing squid pattern. `NihongoSidebar.tsx` remains in the repo unused pending a follow-up cleanup pass. This was validated locally (UAT) before rollout; production deployment follow-up and a full content-level redesign pass of the remaining lesson/quiz/flashcard pages remain future work.
...
- Content-level redesign pass for the remaining Nihongo pages (curriculum lesson detail, quiz, flashcards, tutor, reading, listening) to fully match the new bottom-nav shell's minimalist tone; current pass covered the shell and dashboard only.
- Remove the now-unused `NihongoSidebar.tsx` and `MobileSidebarDrawer.tsx` once confident no other route references them.
